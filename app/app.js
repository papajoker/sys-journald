...
ipcMain.on(consts.events.JOURNALCTL, (event, query) => {
    ...
    exec(command, {
        ...
        if (plus) {
            fs.writeFile(`${consts.tmp}stdout.json`, stdout, 'utf8', (err, data) => {
...
ipcMain.on(consts.events.PLOT, (event) => {
    // add un ENV sans couleurs
    const src = `${consts.tmp}plot.svg`
    exec(`systemd-analyze plot > ${src}`,
        (err, stdout, stderr) => {
            if (!err) {
                event.sender.send(consts.events.PLOT_REPLY, src )
            } else {
                console.log('bash ERROR', stderr)
            }
        })
    ...
    exec(bash, (err, stdout, stderr) => {
        if (!err) {
            ...
            try {
                ...
                event.sender.send(consts.events.LIST_RUN_UNITS_REPLY, response)
            } catch (e) {
...
ipcMain.on(consts.events.BASH_DF, (event, item) => {
    exec(`df -lh --output="source,fstype,size,used,avail,pcent,target" -x tmpfs -x devtmpfs| tail -n +2`, (err, stdout, stderr) => {
        ...
        exec(`lsblk --output="NAME,SIZE,MOUNTPOINT" -lJ`, (err, stdout, stderr) => {
